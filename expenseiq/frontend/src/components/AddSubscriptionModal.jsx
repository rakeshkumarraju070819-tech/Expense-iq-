...
const CATEGORIES = [
  { id: "Entertainment", label: "Entertainment", icon: "📺", color: "bg-pink-500" },
  { id: "Software", label: "Software", icon: "📄", color: "bg-indigo-500" },
  { id: "Utilities", label: "Utilities", icon: "🛜", color: "bg-amber-500" },
  { id: "Health", label: "Health", icon: "⚕️", color: "bg-emerald-500" },
  { id: "Shopping", label: "Shopping", icon: "🛒", color: "bg-blue-500" },
...
export default function AddSubscriptionModal({ onAdd, onClose }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.title || !formData.amount || !formData.nextDueDate) return;
    onAdd(formData);
  };

  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden bg-white shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto custom-scrollbar">
          <div className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1.5">Next Due Date</label>
              <input
                type="date"
